...
[Lexer](#Lexer)|[wikipedia](https://en.wikipedia.org/wiki/Lexical_analysis)|need to only expend $ in double and not quote (hashmap) and check for lexing logic at end (also check all ft_calloc for potentials fails)|
...
`cat < make*` => with one file were its name starts by `make` it'll work. If we have multiple files, that just not working and send an error `bash: make*: ambiguous redirect` :white_check_mark: :warning: The wildcard works with directory.
